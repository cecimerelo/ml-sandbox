...
class TestScreenOne:
    def test_keeps_a_small_tabular_dataset(self):
        assert screen_one(make(rows=200, features=10)) is None

    def test_rejects_above_the_feature_cap(self):
        reason = screen_one(make(features=MAX_FEATURES + 1))
        assert reason is not None
        assert "NFR-2" in reason

    def test_keeps_exactly_at_the_feature_cap(self):
        assert screen_one(make(features=MAX_FEATURES)) is None

    def test_rejects_image_derived_even_under_the_cap(self):
        # The cap would normally catch these; the explicit list keeps the intent visible
        # and catches any that would slip under it.
    ...
    def test_require_small_rejects_datasets_already_covered_by_the_suites(self):
        reason = screen_one(make(rows=SMALL_BAND_MAX_ROWS), require_small=True)
        assert reason is not None
        assert "already covered" in reason

    def test_require_small_keeps_just_below_the_threshold(self):
        assert screen_one(make(rows=SMALL_BAND_MAX_ROWS - 1), require_small=True) is None

    def test_large_datasets_are_fine_when_small_is_not_required(self):
...
class TestFamilies:
    ...
    def test_relatives_share_a_family(self, left, right):
        assert family_of(left) == family_of(right)

    def test_unrelated_datasets_do_not_collide(self):
        assert family_of("credit-g") != family_of("diabetes")

    def test_only_one_member_of_a_family_survives(self):
        screening = screen(
            [
    ...
    def test_family_slot_is_not_spent_on_a_dataset_that_fails_anyway(self):
        # The oversized member must not occupy the family's single slot and push out a
        # usable sibling.
        screening = screen(
            [
...
class TestScreening:
    def test_every_candidate_is_either_kept_or_explained(self):
        candidates = [
...
class TestCoverage:
    def test_reports_a_zero_for_an_uncovered_band(self):
        # The finding that started D-006: the curated suites contain nothing below 500
        # rows, so the recommender's most distinctive advice would go unvalidated.
        report = coverage([make(1, "a", rows=2000), make(2, "b", rows=50_000)])
        assert report["rows < 500"] == 0

    def test_counts_characteristics_the_recommender_reasons_about(self):
        report = coverage(
            [
